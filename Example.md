...
**目的**  
基于一个已完成的过渡态优化输出文件（`.log`），生成一个高水平的单点能计算输入文件（`.gjf`），例如将 B3LYP/6‑31G(d) 提升到 CCSD(T)/aug‑cc‑pVTZ 水平。同时演示如何为 IRC 任务批量生成输入文件。
...
6. 转换成功后，在源文件同目录下会生成一个新文件，命名如 `high-ts_b3lyp-sp.gjf`。若启用了“自动打开文件”，文件夹会自动弹出。

   > *（在此处插入“生成的 gjf 文件内容”截图）*
